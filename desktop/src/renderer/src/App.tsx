    ...
    <main className="mx-auto max-w-[1180px] px-8 py-8">
      <Tabs value={tab} onValueChange={setTab}>
        ...
        {developerMode && <TabsContent value="developer" className="grid gap-5 lg:grid-cols-2">
          ...
          <Card><CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheck />开发诊断</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><div className="flex justify-between rounded-lg border border-white/10 p-4"><span className="text-zinc-400">显示器配置表</span><span>{connection.profileName}</span></div>{([['亮度','brightness'],['音量','volume'],['静音','mute'],['输入源','input']] as const).map(([label, control]) => <div key={control} className="flex justify-between rounded-lg border border-white/10 p-4"><span className="text-zinc-400">{label}路径</span><span>{transportLabel(connection.routes[control])}</span></div>)}<div className="flex justify-between rounded-lg border border-white/10 p-4"><span className="text-zinc-400">USB 候选过滤</span><span>Espressif + MAC</span></div><div className="flex justify-between rounded-lg border border-white/10 p-4"><span className="text-zinc-400">刷写前校验</span><span>ESP32-S3 + MAC 一致</span></div></CardContent></Card>
        </TabsContent>}
      </Tabs>
    </main>
  </div>
