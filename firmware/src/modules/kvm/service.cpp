...
bool backend_address_was_discovered = false;
...
String discoverySignature(const String &message) {
  ...
  if (!info || mbedtls_md_hmac(
                   ...
                   reinterpret_cast<const unsigned char *>(
                       remote_config.token.c_str()),
                   remote_config.token.length(),
...
bool constantTimeEqual(const String &left, const String &right) {
  ...
  for (size_t index = 0; index < left.length(); ++index) {
    different |= static_cast<uint8_t>(left[index] ^ right[index]);
  }
  return different == 0;
}
...
bool wifiRequest(const char *method, const String &path, const char *body,
  ...
  if (!isLocalBackendHost(remote_config.host)) {
    Serial.println("HTTP_FAIL,status=non-local-backend-blocked");
    return false;
  }
  WiFiClient client;
  HTTPClient http;
...
void sendDiscoveryReply(const IPAddress &address, uint16_t port,
                        const String &unsigned_message) {
  String message = unsigned_message + "|" +
                   discoverySignature(unsigned_message);
...
void sendCoordinationBroadcast(const String &unsigned_message) {
...
void handlePassiveDiscovery() {
  ...
  if (read <= 0) return;
  String wire(buffer);
  int signature_separator = wire.lastIndexOf('|');
  if (signature_separator < 0) return;
  String unsigned_message = wire.substring(0, signature_separator);
  String supplied_signature = wire.substring(signature_separator + 1);
  if (!constantTimeEqual(discoverySignature(unsigned_message),
                         supplied_signature)) {
    return;
  }

  if (wireField(wire, 0) == "AZORIA_DESKTOP_HEARTBEAT_V1" &&
      wireFieldCount(wire) == 7) {
    ...
    if (reachable) {
      ...
      backend_address_was_discovered = true;
    }
    return;
  }

  if (wireField(wire, 0) == "AZORIA_DESKTOP_RESULT_V1" &&
      wireFieldCount(wire) == 10) {
...
void invalidateDiscoveredBackend() {
  if (!backend_address_was_discovered) return;
  remote_config.host = "";
  backend_address_was_discovered = false;
}
...
void registerDevice() {
  ...
  if (request("POST", "/v1/device/register", body, response)) {
    Serial.printf("Registered with backend as %s\n", device_id.c_str());
  }
}
...
bool readStatus() {
  ...
  if (!request("GET", "/v1/status", nullptr, response,
    ...
    updateReady(false, false, "Backend offline");
    return false;
  }
  xSemaphoreTake(state_mutex, portMAX_DELAY);
...
void remoteTask(void *) {
  ...
  for (;;) {
    ...
    if (!backend_verified) {
      ...
      if (!readStatus()) {
        if (!bleTransportReady()) invalidateDiscoveredBackend();
